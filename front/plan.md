...
**ВАЖНО:** При реализации каждого этапа необходимо писать подробные комментарии в коде и давать развёрнутые объяснения в чате, как будто ты - senior и объясняешь джуну.

### Комментарии в коде

1. **Комментарии к декораторам и декорациям:**
   - Объяснять назначение каждого декоратора (`@Input()`, `@Output()`, `@Component()`)
   - Пояснять параметры декораторов (`required: true`, `standalone: true`)
   - Сравнивать с аналогами в React (если уместно)

2. **Комментарии к свойствам и методам:**
   - Объяснять назначение каждого свойства класса
   - Описывать логику методов
   - Указывать, когда и зачем вызывается метод
   - Отмечать TODO для будущих этапов

3. **Комментарии в HTML шаблонах:**
   - Объяснять синтаксис Angular (`@for`, `@if`, `[property]`, `(event)`)
   - Пояснять директивы и их параметры
   - Объяснять важные концепции (например, почему `track` обязателен в `@for`)


### Объяснения в чате

1. **Перед началом реализации:**
   - Кратко описать, что будет сделано на этапе
   - Объяснить ключевые концепции, которые будут использованы

2. **Во время реализации:**
   - Объяснять каждый шаг подробно
   - Сравнивать с React (если уместно)
   - Объяснять, почему делается именно так
   - Предупреждать о типичных ошибках

3. **После реализации:**
   - Резюмировать выполненную работу
   - Выделять ключевые концепции Angular
   - Объяснять, что будет на следующем этапе

### Примеры хороших комментариев

```typescript
// @Input() - это декоратор, который позволяет родительскому компоненту
// передавать данные в дочерний компонент.
// В React это было бы просто props, но в Angular нужно явно пометить свойство.
// Тип Chat гарантирует, что мы получим правильную структуру данных.
...
```css
/* 
  :hover - псевдокласс для состояния при наведении мыши.
  transition выше делает изменение плавным (0.2s ease).
*/
.chat-item:hover {
...
- **Сравнения:** Использовать сравнения с React для лучшего понимания
- **Примеры:** Приводить примеры использования и типичные ошибки
- **Структура:** Комментарии должны быть структурированы и читаемы
