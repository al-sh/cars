...
1. **max_price** (обязательный)
2. **Минимум 2 дополнительных параметра** (body_type, engine_type, transmission, seats, drive, brand, year_min/max)
...
**Что происходит:**
1. LLM извлекает критерии → `ready_to_search: true`
...
2. **Близость к цене** (не показывать на 30%+ дороже max_price)
