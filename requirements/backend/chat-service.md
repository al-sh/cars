...
### Генерация title чата

**Когда вызывается:**
- После первого `user` message в чате, если `chat.title == null`
- Вызывается асинхронно (`@Async`) из `MessageService.sendMessage()`, чтобы не блокировать основной поток обработки сообщения

**Что делает:**
1. `LLMService.generateTitle(firstMessage)` отправляет первое сообщение пользователя в LLM с промтом "Придумай короткое название (3-5 слов)"
2. Полученный title сохраняется в `chats.title` через `chatRepository.updateTitle(chatId, title)`

**Условия:**
- Генерация происходит только если `chat.title == null` (title не был задан пользователем вручную)
- Если пользователь уже переименовал чат (через PATCH /chats/{id}), генерация **не** перезаписывает title

**Как фронтенд узнаёт об обновлении:**
- Через SSE событие `title_updated` в потоке ответа на первое сообщение:
...
- Событие `title_updated` отправляется после того, как title успешно сохранён в БД
- Если генерация title завершилась с ошибкой (LLM timeout и т.п.), событие не отправляется, title остаётся null -- это не критичная ошибка

**Frontend обработка:**
- При получении `title_updated` обновить title чата в `chatsSignal`
- Отобразить обновлённый title в sidebar (ChatList) и в ChatHeader

**Промт:**
```
Системный: "Придумай короткое название (3-5 слов) для чата о подборе автомобиля.
